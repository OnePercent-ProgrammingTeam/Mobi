...
public class ContainerMonitorHttp extends ContainerHttpRequest {
    ...
    /**
     * Get information about a container that might be or might not be locally
     * installed
     *
     */
    public void getContainerInformation() {
        String message = "json"; // get the container statistics in json format
        ContainerMonitorHttp.containerId = Test
                .handleInput("Please type the container ID to get info about the container: ");
        getRequest = new HttpGet(
                ContainerMonitorHttp.DOCKER_HOST + "/containers/" + ContainerMonitorHttp.containerId + "/" + message);
        System.out.println("Follow the link for " + message + " info:\n" + "LINK: " + ContainerMonitorHttp.DOCKER_HOST
                + "/containers/" + ContainerMonitorHttp.containerId + "/" + message + "\n\n");
    ...
    public CloseableHttpResponse getContainerStats() {
        ...
        ContainerMonitorHttp.containerId = Test
                .handleInput("Please type the container ID to plot diagram with CPU usage: ");
        getRequest = new HttpGet(
                ContainerMonitorHttp.DOCKER_HOST + "/containers/" + ContainerMonitorHttp.containerId + "/" + message);
        System.out.println("Follow the link for " + message + " info:\n" + "LINK: " + ContainerMonitorHttp.DOCKER_HOST
                + "/containers/" + ContainerMonitorHttp.containerId + "/" + message + "\n\n");
     ...
     * @throws NullPointerException if an error occurs while executing the http
     *                              request
     *
     *                              The code below should work too. Instead, it
     *                              return 0.0. We have to check it further
     * 
     *                              <code>
    ...
    public void printFormattedJson() throws JsonProcessingException {
        try {
            ...
            System.out.println("Network ID: "
                    + jsonNode.get("NetworkSettings").get("Networks").get("bridge").get("NetworkID").asText());
            System.out.println("Gateway: "
                    + jsonNode.get("NetworkSettings").get("Networks").get("bridge").get("Gateway").asText());
            System.out.println("IP Address: "
                    + jsonNode.get("NetworkSettings").get("Networks").get("bridge").get("IPAddress").asText());
            System.out.println("Mac Address: "
                    + jsonNode.get("NetworkSettings").get("Networks").get("bridge").get("MacAddress").asText());
            // System.out.println("Image Name: " +
            // jsonNode.get("Config").get("Labels").get("org.opencontainers.image.title").asText());
        } catch (NullPointerException e) {
            System.out.println("------------------------------------------");
        }
    }
    ...
    public String[] prepareStorageData() {
        try {
            ...
            getRequest = new HttpGet(
                    ContainerMonitorHttp.DOCKER_HOST + "/containers/" + ContainerMonitorHttp.containerId + "/json");
            ...
            ContainerHttpRequest.getRequest = new HttpGet(
                    ContainerMonitorHttp.DOCKER_HOST + "/containers/" + ContainerMonitorHttp.containerId + "/stats");
    ...
    /**
     * Format the json response for image -that you 've searched for- to a
     * user-friendly message
     * 
     * @throws JsonProcessingException
     * @throws NullPointerException
     * @prints info about the top 3 images with the name that user searched. The
     *         info contains:
     *         1.Image Name
     *         2.Description
