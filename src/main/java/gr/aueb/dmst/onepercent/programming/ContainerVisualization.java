...
public class ContainerVisualization extends JFrame {
     ...
     * @variable dataset a collection of XYSeries objects that can be used as a
     *           dataset
     */
    public ContainerVisualization(String title) {
        ...
        XYPlot plot = (XYPlot) chart.getPlot(); // Get the plot from the chart, casting it to XYPlot because it may be a
                                                // subclass
        DateAxis dateAxis = (DateAxis) plot.getDomainAxis(); // Get the domain axis from the plot, casting it to
                                                             // DateAxis because it may be a subclass
        dateAxis.setTickUnit(new DateTickUnit(DateTickUnitType.SECOND, 1)); // Set the tick unit to be 1 second (it is a
                                                                            // real time chart, so we want to update
                                                                            // every second)
    ...
    private void updateStats(long timestamp, double usage) {
        ...
        long timestampInMilliseconds = new Date(timestamp).getTime(); // Convert the timestamp to milliseconds, type
                                                                      // long
        usageSeries.addOrUpdate(timestampInMilliseconds, usage);
        /*
         * addOrUpdate() method will add a new data point if it doesn't exist, or update
         * the existing one if it does
         * the parameters it gets are the x and y values of the data point (should be
         * double)
         */
    }

    /**
     * Start updating the stats in order to plot real time data-the consume of cpu
     * resources every second
     */
    public void startUpdatingStats(CloseableHttpResponse response, ContainerVisualization ex)
     ...
     */
    public void onRunGraph(boolean flag, BufferedReader reader, ContainerVisualization ex, Timer timer)
            throws IOException {
        ...
        if (inputLine != null) {
            ...
            if (!flag) {
                // ex.setVisible(true); // Make the window visible to the end user, we use flag
                // because we want to make the window visible only once
                flag = true;
            }
        } else {
    ...
    public static void main(String[] args) {
        ...
        ContainerVisualization example = new ContainerVisualization("Container Stats Plotter"); // Create a new
        ...
        example.setDefaultCloseOperation(ContainerVisualization.DO_NOTHING_ON_CLOSE); // Set the close operation, so
                                                                                      // that the application exits when
